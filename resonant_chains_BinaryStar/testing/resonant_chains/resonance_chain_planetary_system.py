...
if __name__ in ('__main__', '__plot__'):
    ...
    bodies = read_set_from_file("../data/config_1_vh/Nbody_data/initial_particles/initial_conds.hdf5")

    stable_cluster = Particles()
    stable_cluster.add_particles(bodies[bodies.syst_id < 0])
    for syst_id in np.unique(bodies.syst_id):
        if syst_id > 0:
            ...
            if len(system) > 2:
                ...
                print(original_pos.in_(units.pc))
                print(original_vel.in_(units.kms))
                for pl in p:
                ...
                print(f"PLANET ORIGINAL SYSTEM")
                print(host.position.in_(units.pc))
                print(host.velocity.in_(units.kms))
                print(":"*50)
            stable_cluster.add_particles(system)
    #else:
    #    bodies = resonant_pair_planetary_system(bodies, o.inner_planet_id, o.outer_planet_id,
    #                                            o.tau_a_factor,
    #                                            o.t_integration, o.n_steps)
    write_set_to_file(stable_cluster,
                      "resonant_system.hdf5",
                      overwrite_file=True,
